...
class Parse_MF:
    ...
    def __init__(self, data):
        self.data = data
        self.monoPPM = self.data.ppm_values[2]
    ...
    def run_all(self):
        for x, y in self.mass_df.items():
            drift_cal = Drift_Time(self.mzml, x, y.values, list(y.index), self.monoPPM, self.isotope, self.primary_ion, self.temp_spectrum, self.temp_drift, self.data.message)
            drift_cal.launch()
...
class Drift_Time:
    ...
    def __init__(self, mzml, molecular_formula, mass_values, ions, PPM, isotope, primary_ion, temp_spectrum, temp_drift, message):
        ...
        for i, spec in enumerate(spec_one):
            ...
            if bool(ff):
                for t, p in enumerate(target):
                    condition = np.absolute(ff.mz- p) < self.PPM/1000000 * p
    ...
    def plotrt_EIC(self):
        ...
        for x in self.rt_found_ion:
            ...
            for ion in ions:
                ...
                xnew = np.arange(df['rt'].min(), df['rt'].max(), 0.05)
                ynew = f(xnew)
    ...
    def rt_window_formula(self):
        ...
        final_index, rr = self.spectrum_confirm(spec_index, self.primary_ion, 5)
    ...
    def spectrum_confirm(self, spec_index, ion_type, num):
        run = pymzml.run.Reader(self.mzml,  MS1_Precision=8e-6)
        ...
        for spec in spec_peaks:
            ff= self.extract_decorator(spec)
            condition = np.absolute(ff.mz- target1) < self.PPM/1000000 * target1
            result_mz1 = np.extract(condition, ff.mz)
            result_intensity1 = np.extract(condition, ff.intensity)
            condition = np.absolute(ff.mz- target2) < self.PPM/1000000 * target2
        ...
        for ion in self.mass_all.keys():
            ...
            for i in range(len(pp)):
                ...
                if ll.index.size > 5:
                    ...
                    ratio_bool, _ = self.spectrum_confirm(spec_index, ion, num)
